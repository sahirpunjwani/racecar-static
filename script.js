...
function setHorn(on) { 
    if (hornGain && gameActive) hornGain.gain.setTargetAtTime(on ? 0.15 : 0, audioCtx.currentTime, 0.02); 
}
...
function buildTrack() {
    ...
    function addFloor(num, curve, hill) {
        for (let n = 0; n < num; n++) {
            segments.push({
                index: segments.length, world: { x: 0, y: lastY(), z: segments.length * SEGMENT_L }, curve: curve, hill: hill,
    ...
    for (let i = 0; i < totalCars; i++) {
        ...
        cars.push({
            position: spawnPos,
            targetLaneX: lanePositions[laneIdx], 
            x: lanePositions[laneIdx],
            lane: laneIdx,
            heading: isSouthbound ? 'south' : 'north',
            baseSpeed: isSouthbound ? (MAX_SPEED * 0.25) : (MAX_SPEED * 0.35 + Math.random() * 2000),
            speed: isSouthbound ? (MAX_SPEED * 0.25) : (MAX_SPEED * 0.35 + Math.random() * 2000),
            color: isSouthbound ? '#ff3366' : '#ffff00',
            width: 0.25,
            isEvading: false,   
            evadeTimer: 0
        });
...
function startGame() {
    ...
    buildTrack();
    position = 0; playerX = 0.5; speed = 3000; score = 0;
    screenShake = 0; crashSpin = 0; crashTimer = 0; carRotation = 0; carLean = 0;
...
function update(dt) {
    ...
    if (playerX < -1.6) playerX = -1.6; if (playerX > 1.6) playerX = 1.6;
    if ((playerX < -1.15) || (playerX > 1.15)) { if (speed > 2000) speed += OFF_ROAD_DECEL * dt; }

    cars.forEach(car => {
        let oldSeg = findSegment(car.position);
        
        if (car.heading === 'south') {
            car.position = (car.position - car.speed * dt + trackLength) % trackLength;
        } else {
        ...
        if (car.heading === 'north') {
            ...
            if (relativeDist > (trackLength - 1200) && relativeDist < trackLength && Math.abs(playerX - car.x) < 0.4) {
                ...
                car.lane = (playerX > 0) ? 2 : 3;
            } else {
                car.speed = car.baseSpeed;
            }

            if (keys.space) {
                let distanceAhead = car.position - position;
                if (distanceAhead < 0) distanceAhead += trackLength;

                if (distanceAhead > 0 && distanceAhead < 3500 && Math.abs(playerX - car.x) < 0.35) {
                    car.isEvading = true;
                    car.evadeTimer = 2.0; 
                    if (car.lane === 2) car.targetLaneX = 0.75; 
                    else if (car.lane === 3) car.targetLaneX = 0.25;
                }
            }
        }
        ...
        if (car.isEvading) {
            ...
            car.evadeTimer -= dt;
            if (car.evadeTimer <= 0) car.isEvading = false;
        } else {
            const defaultLanes = [-0.75, -0.25, 0.25, 0.75];
            car.targetLaneX = defaultLanes[car.lane];
...
function updateParticles(dt) {
    particles.forEach((p, i) => {
        p.x += p.vx; p.y += p.vy; p.alpha -= dt * 1.5;
...
function drawTrafficCar(x, y, scale, color, isOncoming) {
    const carW = Math.round(440 * scale * (WIDTH / 2));
    const carH = Math.round(220 * scale * (WIDTH / 2));
    if (carW < 2) return;

    ctx.save();
    ctx.translate(x, y - carH);
    ...
    ctx.fillStyle = isOncoming ? '#ffffff' : '#ff0033'; 
    ctx.shadowColor = ctx.fillStyle;
    ctx.fillRect(-carW/2 + 3, carH - carH*0.3, carW*0.16, carH*0.15);
    ctx.fillRect(carW/2 - 3 - carW*0.16, carH - carH*0.3, carW*0.16, carH*0.15);
    ctx.restore();
}
...
function drawPlayerCar() {
    ...
    ctx.beginPath(); ctx.moveTo(-26, -2); ctx.lineTo(-16, -26); ctx.lineTo(16, -26); ctx.lineTo(26, -2);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ...
    if (speed > 0 || crashTimer > 0) {
        ...
        ctx.fillRect(-38 + carLean, 20, 16, tH);
        ctx.fillRect(22 + carLean, 20, 16, tH);
    }
    ctx.restore();
}
